...
/**
 * The {@code NormalMutator} class performs the mutation of a {@link NumericGene}.
	...
	extends Mutator<G>
{
	...
	public NormalMutator() {
	}

	public NormalMutator(final double probability) {
		super(probability);
	}
	...
	G mutate(final G gene, final Random random) {
		...
		return gene.newInstance(math.clamp(
			random.nextGaussian() * std + gene.getAllele().doubleValue(),
