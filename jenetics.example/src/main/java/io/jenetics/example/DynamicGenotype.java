...
public class DynamicGenotype {
		...
		extends AbstractAlterer<G, C>
	{
		...
		public AlterResult<G, C> alter(
			...
			random.indexes(RandomRegistry.getRandom(), pop.size(), p).forEach(i -> {
		...
		private Genotype<G> mutate(
			...
			alterations.value +=
				io.jenetics.internal.math.random.indexes(RandomRegistry.getRandom(), chromosomes.size(), p)
		...
		private int mutate(final List<G> genes, final double p) {
			...
			return (int) io.jenetics.internal.math.random.indexes(random, genes.size(), p)
