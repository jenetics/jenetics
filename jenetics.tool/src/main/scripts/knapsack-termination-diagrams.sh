...
RESULTS=(
	...
	"Knapsack-population_convergence_termination.xml:Generation,Fitness"
	#"Knapsack-selector_comparison.xml:Fitness1,Fitness2"
	"Knapsack-steady_fitness_termination.xml:Generation,Fitness"
)

SCRIPT_DIR=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)
RESULT_BASE_PATH=`read_link "${SCRIPT_DIR}/../results/io/jenetics/tool/evaluation"`
...
do
	...
	SAMPLES="${item#*:}"
	${JRUN} io.jenetics.tool.evaluation.Diagram --input ${RESULT} --samples ${SAMPLES}
