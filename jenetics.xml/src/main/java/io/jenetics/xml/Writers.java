...
public final class Writers {
	...
	public static final class BitChromosome {
		...
		public static Writer<io.jenetics.BitChromosome> writer() {
			return Writer.elem(ROOT_NAME,
				Writer.attr(LENGTH_NAME).map(io.jenetics.BitChromosome::length),
				Writer.attr(ONES_PROBABILITY_NAME).map(ch -> ch.getOneProbability()),
				Writer.text().map(io.jenetics.BitChromosome::toCanonicalString)
			);
		}
	...
	public static final class CharacterChromosome {
		...
		public static Writer<io.jenetics.CharacterChromosome> writer() {
			return Writer.elem(ROOT_NAME,
				Writer.attr(LENGTH_NAME).map(io.jenetics.CharacterChromosome::length),
				Writer.elem(VALID_ALLELES_NAME,
					Writer.text().map(ch -> ch.getGene().getValidCharacters())),
				Writer.elem(ALLELES_NAME,
					Writer.text().map(io.jenetics.CharacterChromosome::toString))
			);
		}
	...
	public static final class BoundedChromosome {
		...
		Writer<C> writer(
			...
			return Writer.elem(rootName,
				Writer.attr(LENGTH_NAME).map(ch -> ch.length()),
				Writer.elem(MIN_NAME, alleleWriter.map(ch -> ch.getMin())),
				Writer.elem(MAX_NAME, alleleWriter.map(ch -> ch.getMax())),
				Writer.elem(ALLELES_NAME,
					Writer.elems(ALLELE_NAME, alleleWriter)
	...
	public static final class DoubleChromosome
		//extends WriterProvider<DoubleChromosome>
	{
	...
	public static final class PermutationChromosome {
		...
		writer(final Writer<? super A> alleleWriter) {
			return Writer.elem(ROOT_NAME,
				Writer.attr(LENGTH_NAME).map(io.jenetics.PermutationChromosome::length),
				Writer.elem(VALID_ALLELES_NAME,
					Writer.attr("type").map(PermutationChromosome::toAlleleTypeName),
					Writer.elems(ALLELE_NAME, alleleWriter)
						.map(ch -> ch.getValidAlleles())
				),
				Writer.elem(ORDER_NAME, Writer.text())
					.map(ch -> ch.stream()
		...
		public static <A> void write(
			...
			throws XMLStreamException
		{
			write(out, data, null, Writer.text());
		}
	...
	public static final class Genotype {
		...
		Writer<io.jenetics.Genotype<G>> writer(final Writer<? super C> writer) {
			return Writer.elem(
				ROOT_NAME,
				Writer.attr(LENGTH_NAME).map(io.jenetics.Genotype<G>::length),
				Writer.attr(NGENES_NAME).map(io.jenetics.Genotype<G>::getNumberOfGenes),
				Writer.elems(writer).map(gt -> cast(gt.toSeq()))
			);
		}
	...
	public static final class Genotypes {
		...
		writer(final Writer<? super C> writer) {
			return Writer.elem(
				ROOT_NAME,
				Writer.attr(LENGTH_NAME).map(Collection::size),
