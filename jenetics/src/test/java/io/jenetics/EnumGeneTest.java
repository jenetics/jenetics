...
import io.jenetics.util.factories;
...
public class EnumGeneTest extends GeneTester<EnumGene<Integer>> {
	...
	_factory = new Factory<EnumGene<Integer>>() {
		private ISeq<Integer> _alleles = MSeq.<Integer>ofLength(100).fill(factories.Int()).toISeq();
	...
	public void valueOf() {
		...
		final ISeq<Integer> alleles = MSeq.<Integer>ofLength(length).fill(factories.Int()).toISeq();
