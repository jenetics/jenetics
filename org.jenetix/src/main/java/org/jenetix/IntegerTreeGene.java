		...
		Serializable
{
	...
	of(final int value, final IntRange range, final int... children) {
		return new IntegerTreeGene(value, range.getMin(), range.getMax(), children);
	}
	 ...
	 * @throws NullPointerException if one of the arguments is {@code null}.
	 */
	public static IntegerTreeGene of(final IntRange range, final int... children) {
		return of(nextInt(getRandom(), range.getMin(), range.getMax()), range, children);
	}
